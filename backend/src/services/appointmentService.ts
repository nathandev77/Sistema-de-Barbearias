...
export class AppointmentService {
  static async createAppointment(data: any, tenantId: string) {
    ...
    if (!client || !barber || services.length !== data.serviceIds.length) {
      throw new Error('Violação de Segurança: Referência inválida ou pertencente a outra barbearia.');
    }
    ...
    if (dailyAppointments >= 1) {
      throw new Error('Este cliente já possui um agendamento marcado para este dia.');
    }
  ...
  static async listAppointments(tenantId: string) {
  ...
  static async updateAppointment(id: string, data: any, tenantId: string) {
    ...
    const appointment = await prisma.appointment.update({
      ...
      data: {
        ...updateData,
        ...(serviceIds ? { services: { set: serviceIds.map((id: string) => ({ id })) } } : {})
      }
    });
  ...
  static async deleteAppointment(id: string, tenantId: string) {
  ...
  static async getAvailableSlots(barberId: string, date: string, tenantId: string) {
